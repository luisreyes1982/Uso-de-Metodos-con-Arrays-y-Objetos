...
//Requerimiento 1.- Ingreso de horas al arreglo de Traumatología 
traumatologia.push(
...
// Requerimiento 2
//Método shift elimina el primer objeto del arreglo
radiologia.shift();
//Método pop elimina el último objeto del arreglo
radiologia.pop();
...
dental.forEach(function (test) {
    console.log(`${test.hora} - ${test.especialista} - ${test.paciente} - ${test.rut} - ${test.prevision}`);

    document.write(`${test.hora} - ${test.especialista} - ${test.paciente} - ${test.rut} - ${test.prevision} <br><br>`)

})

// 4.- Imprimir un listado total de todos los pacientes que se atendieron en el centro médico
function buscar() {
    let servicios = radiologia.concat(traumatologia, dental);
    console.log(servicios);
    servicios.forEach(function (servicios) {
        const pac = `el nombre de los pacientes :  ${servicios.paciente} `;
        document.write('<p> </p>', pac)

    })
}

// 5.- Filtrar pacientes de ISAPRE de Dental
let isapredental = []
dental.forEach(dato => {
    if (dato.prevision == 'ISAPRE') {
        document.write(`<p>${dato.paciente} - ${dato.prevision}</p>`);
    }
});

document.write('<hr>');

//6.- Filtrar pacientes de Fonasa de Traumatologia
let fonasatrauma = []
traumatologia.forEach(dato => {
